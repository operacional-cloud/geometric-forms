      ...
      <main className="min-h-screen flex items-center justify-center px-4 py-10" style={{ background: '#0f0f12' }}>
        ...
        >
          <div className="rounded-2xl overflow-hidden border border-white/10" style={{ background: '#16161B' }}>
            {form.cover_image_url ? (
              ...
              >
                {tenant.logo_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={tenant.logo_url} alt={tenant.name} className="h-16" />
                ) : (
                  <span className="font-display text-3xl tracking-tightest" style={{ color: accent }}>
                    {tenant.name}
                  </span>
                )}
              </div>
          ...
          <div className="mt-6 text-center px-2">
            <h1 className="text-2xl font-bold leading-tight">{form.title}</h1>
            {form.description && (
              <p className="mt-4 text-fg-muted text-sm leading-relaxed">{form.description}</p>
            )}
          </div>
          ...
          <button
            type="button"
            onClick={() => setStep(0)}
            className="mt-8 w-full rounded-full py-4 font-semibold text-base inline-flex items-center justify-center gap-2 transition-transform active:scale-95"
        ...
        >
          ...
          {wa ? (
            <a
              ...
              className="mt-8 w-full rounded-full py-4 font-semibold text-base inline-flex items-center justify-center transition-transform active:scale-95"
              style={{ background: accent, color: accentText }}
            >
              {buttonLabel}
            </a>
            ...
            <div
              className="mt-8 inline-block rounded-full px-6 py-3 text-sm"
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 py-10" style={{ background: '#0f0f12' }}>
      <div className="w-full max-w-md">
        <div className="rounded-2xl border border-white/10 p-6 lg:p-8" style={{ background: '#16161B' }}>
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <button
            ...
            onClick={goPrev}
            disabled={step === 0}
            className="text-sm font-medium text-fg-muted disabled:opacity-30 hover:text-fg transition-colors py-2 px-3"
          >
            Voltar
          </button>
          <button
            ...
            onClick={goNext}
            disabled={busy}
            className="rounded-full px-7 py-3 font-semibold text-sm transition-transform active:scale-95 disabled:opacity-50"
            style={{ background: accent, color: accentText }}
          >
        ...
        <div className="space-y-2.5">
          {field.options?.map((o, i) => {
            ...
            return (
              ...
              >
                <span
                  className="shrink-0 h-7 w-7 rounded-full border flex items-center justify-center text-xs font-bold"
                  style={
                    checked
        ...
        <div className="space-y-2.5">
          {field.options?.map((o, i) => {
            ...
            return (
              ...
              >
                <span
                  className="shrink-0 h-7 w-7 rounded-md border flex items-center justify-center text-xs font-bold"
                  style={
                    checked
                      ? { borderColor: accentText, color: accentText, background: 'transparent' }
                      : { borderColor: 'rgba(255,255,255,0.25)', color: 'rgba(255,255,255,0.65)' }
